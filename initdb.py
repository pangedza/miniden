...
Используется для ручного запуска или dev-инициализации.
"""
...
def init_db_from_env(env_flag: str = "MINIDEN_INIT_DB") -> bool:
    return os.getenv(env_flag, "").strip().lower() in {"1", "true", "yes", "on"}


def init_db_if_enabled(env_flag: str = "MINIDEN_INIT_DB") -> None:
    if init_db_from_env(env_flag):
        init_db()
