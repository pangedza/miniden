...
from routes_adminsite import get_router as get_adminsite_router
from routes_auth import router as auth_router
from routes_public import get_router as get_public_router
...
def ensure_admin_static_dirs() -> bool:
    try:
        ...
        logger.warning("Admin static directory is unavailable, skipping mount: %s", exc)
        return False
...
if ensure_admin_static_dirs():
    try:
        ...
        logger.warning("Admin static will not be served because mount failed: %s", exc)
...
app.include_router(get_adminsite_router(app))
app.include_router(get_public_router(BUILD_COMMIT, BUILD_TIME, SERVICE_NAME))
