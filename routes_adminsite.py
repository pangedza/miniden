...
from admin_panel.adminsite import ADMINSITE_CONSTRUCTOR_PATH, ADMINSITE_STATIC_ROOT
from admin_panel.adminsite import service as adminsite_service
from database import get_db, get_session
from media_paths import ADMIN_BOT_MEDIA_ROOT, ADMIN_SITE_MEDIA_ROOT, MEDIA_ROOT, ensure_media_dirs
from models import SupportMessage, SupportMessageList, SupportSessionDetail, SupportSessionList, User
...
from routes_auth import _get_current_user_from_cookie
from routes_public import _faq_to_dict, _validate_category_type, _validate_type
...
def _save_branding_file(
    ...
    target_dir = Path(MEDIA_ROOT) / "adminsite" / "branding"
...
def _delete_media_file(url: str | None) -> None:
    ...
    try:
        if url.startswith("/media/"):
            target_path = Path(MEDIA_ROOT) / url.replace("/media/", "")
        elif url.startswith("/static/"):
            target_path = Path(STATIC_DIR) / url.replace("/static/", "")
        else:
            return
...
def _save_uploaded_image(
    ...
    return {"url": f"/media/{relative}", "path": str(full_path)}
...
async def _parse_category_payload(
    ...
    *,
    ...
    if request.headers.get("content-type", "").startswith("application/json"):
        ...
        raw = dict(form)
        image_file = image_file or form.get("image_file")
    ...
    if not payload.get("name") and not is_update:
        raise HTTPException(status_code=422, detail="name is required")

    return payload, image_file


def _persist_category_image(
    image_file: UploadFile | None, *, current_url: str | None = None
) -> str | None:
    if not image_file:
        return current_url
    ...
    filename = f"{uuid4().hex}{ext}"
    target_dir = ADMIN_SITE_MEDIA_ROOT / "categories"
    target_dir.mkdir(parents=True, exist_ok=True)
    full_path = target_dir / filename
...
def update_site_settings(
    payload: SiteSettingsPayload, request: Request, db: Session = Depends(get_db)
):
    adminsite_service.ensure_admin(request, db)
    return menu_catalog.update_site_settings(payload.model_dump())
...
def admin_menu_items(
    ...
    try:
        ...
        for item in items:
            item["public_url"] = _build_public_link(base_url, f"/i/{item.get('id')}")
        return {"items": items}
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc))
...
def admin_products(user_id: int, type: str = "basket"):
    _ensure_admin(user_id)
    product_type = _validate_type(type)
    items = products_service.list_products_admin(product_type)
    return {"items": items}
...
def admin_update_product(product_id: int, payload: AdminProductsUpdatePayload):
    _ensure_admin(payload.user_id)
    product_type = _validate_type(payload.type)
    updated = products_service.update_product(
        product_type,
        product_id,
        name=payload.name,
        price=payload.price,
...
@router.patch("/api/admin/products/{product_id}/toggle_active")
@router.post("/api/admin/products/{product_id}/toggle")
...
async def admin_create_product_category(
    request: Request, image_file: UploadFile | None = File(None)
):
...
@router.put("/api/admin/product-categories/{category_id}")
async def admin_update_product_category(
    category_id: int, request: Request, image_file: UploadFile | None = File(None)
):
    payload, image_file = await _parse_category_payload(
        request, image_file=image_file, is_update=True
    )
    _ensure_admin(payload["user_id"])
...
@router.post("/api/admin/product-categories/{category_id}/page")
...
def admin_webchat_send(payload: AdminWebChatSendPayload, admin: User = Depends(get_admin_user)):
    if not payload.text:
...
def admin_webchat_close(payload: AdminWebChatClosePayload, admin: User = Depends(get_admin_user)):
    session = webchat_service.get_session_by_id(payload.session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
...
def admin_webchat_reopen(payload: AdminWebChatClosePayload, admin: User = Depends(get_admin_user)):
    session = webchat_service.get_session_by_id(payload.session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
...
def api_admin_webchat_close(session_id: int, admin: User = Depends(get_admin_user)):
    session = webchat_service.get_session_by_id(session_id)
    if not session:
        raise HTTPException(status_code=404, detail="Session not found")
...
def admin_support_messages(user_id: int, session_id: int):
    ...
    messages = webchat_service.get_messages(session, limit=None, mark_read_for="manager")
    return {"items": [_serialize_webchat_message(msg) for msg in messages]}
...
__all__ = ["get_admin_user", "get_router", "router"]
