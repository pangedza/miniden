...
- По клику витрина добавляет позицию в корзину: для Telegram-пользователей делает `POST /api/cart/add` (тип позиции мапится на `basket/course`), иначе сохраняет гостевую корзину в localStorage.
