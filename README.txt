...
- Разрешения для пользователя `miniden` (без запроса пароля):
```
miniden ALL=(root) NOPASSWD: \
  /bin/systemctl start miniden-deploy.service, \
  /bin/systemctl is-active miniden-deploy.service, \
  /bin/systemctl show miniden-deploy.service
```
