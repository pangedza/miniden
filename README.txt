...
- Пользовательская часть сайта авторизуется только через Telegram:
  * если сайт открыт через Telegram WebApp — используется встроенный
    механизм Telegram WebApp (initData), backend авторизует пользователя
    через `/api/auth/telegram`;
  * если сайт открыт в обычном браузере — используется кнопка
    «Войти через Telegram», которая также отправляет данные Telegram
    на `/api/auth/telegram`.
