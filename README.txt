...
- Запуск выполняет только фиксированный серверный скрипт `/opt/miniden/deploy.sh` в фоне; повторный запуск блокируется, если процесс уже идёт (PID сохраняется в `/opt/miniden/logs/deploy.pid`).
- Статус показывает, идёт ли процесс, текущий PID и последние строки лога `/opt/miniden/logs/deploy.log` (папка `/opt/miniden/logs` должна существовать заранее и быть доступной на запись сервису API).
