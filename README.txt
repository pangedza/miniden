...
- **Баннеры (слайдер «Популярный набор»)**
  - создание и редактирование баннеров (заголовок, подзаголовок, текст и ссылка кнопки, URL картинки);
  - сортировка по полю `sort_order` и включение/отключение через `is_active`;
...
- Для карточек товаров и мастер-классов выводится краткое описание (`short_description`). Если поле пустое, на карточке показывается обрезанный вариант `description` (около 140 символов) с многоточием.
- В модальном окне «Подробнее» всегда используется полное описание (`description`); `short_description` служит только запасным вариантом, если полное описание не заполнено.
