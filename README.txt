...
- Если витрина открыта вне Telegram, по клику показывается подсказка «Откройте витрину из Telegram бота», добавление не выполняется.
- По клику витрина делает `POST /api/cart/add` (тип позиции мапится на `basket/course`) и обновляет cart bar.
- Cart bar появляется при наличии позиций, показывает количество и сумму, а кнопка «Оформить» активна только при непустой корзине.
