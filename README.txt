...
Telegram Bot API: create login code
-----------------------------------
- Endpoint для Telegram-бота: `POST /api/bot/auth/create-code`.
- Назначение: бот передаёт телефон и Telegram-пользователя, backend создаёт/обновляет пользователя, инвалидирует предыдущие коды, сохраняет новый код в `login_codes` (в виде hash) и возвращает код в явном виде.
- TTL одноразового кода — 5 минут (`expires_in_seconds = 300`).

Пример запроса:

```bash
curl -X POST http://localhost:8000/api/bot/auth/create-code \
...
- В ответе возвращается только код (без hash) — это допустимо, потому что endpoint предназначен исключительно для бота.
