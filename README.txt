...
Авторизация в Telegram WebApp
-----------------------------
- При открытии мини-приложения из Telegram бот автоматически передаёт в
  веб-приложение `initData`.
- Фронтенд ждёт появления `Telegram.WebApp.initData` (с несколькими попытками)
  и отправляет его на endpoint авторизации через Telegram.
- После успешной авторизации все страницы WebApp используют полученную сессию,
  и пользователь не видит форму «Вы не авторизованы».
- В случае проблем пользователю показывается понятное сообщение и кнопка
  «Повторить авторизацию», которая повторно запускает процедуру авторизации.
