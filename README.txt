...
Stock и кнопка «➕» в Telegram WebApp
------------------------------------
- Товары и курсы используют поле `stock` (по умолчанию 0) в таблицах `products_baskets`, `products_courses`, `adminsite_items`; колонка добавляется при старте через `database.py`.
- API `/api/admin/products` читает/записывает stock (`services/products.py`), а AdminSite-конструктор показывает и сохраняет поле «В наличии (stock)» в модалке и таблице (`admin_panel/adminsite/static/adminsite/modals.js`, `constructor.js`).
- Публичная витрина (`webapp/js/site_app.js`) скрывает кнопку «➕» при stock=0 или вне Telegram WebApp; по клику отправляется `Telegram.WebApp.sendData` с `{action:'add_to_cart', product_id, qty:1}` и окно WebApp закрывается.
- Бот обрабатывает `web_app_data` в `handlers/webapp.py`: логирует, валидирует JSON, добавляет товар через `services/cart.py` и отвечает сообщением «✅ Добавлено в корзину …» с кнопкой «Открыть корзину» (web_app) или подсказкой про кнопку 🛒.
...
Архитектура админки
--------------------
- `/adminbot/*` — фронтенд-маршруты (HTML/JS) веб-админки; их обслуживает статика `webapp/admin.html`.
- `/api/admin/*` — backend API для админки; данные для форм и списков подгружаются через эти эндпоинты.
- JS админки всегда обращается к `/api/admin/...`, чтобы разделить frontend и backend зоны ответственности.
...
Deploy из AdminBot
------------------
- В админском меню Telegram-бота доступны кнопки «🚀 Deploy» (запускает деплой) и «📄 Deploy статус» (показывает состояние и лог).
- Запуск выполняет фиксированный скрипт `/opt/miniden/deploy.sh` в фоне и сразу отвечает PID процесса; повторный запуск блокирует,
  если деплой уже идёт.
- Кнопка статуса читает PID из `/opt/miniden/logs/deploy.pid`, проверяет, жив ли процесс, и отдаёт хвост `/opt/miniden/logs/deploy.log`.
- Функциональность доступна только администраторам AdminBot; путь до скрипта зашит в коде и не параметризуется.
- Логи деплоя пишутся в `/opt/miniden/logs/deploy.log`, PID — в `/opt/miniden/logs/deploy.pid` (папку создавайте вручную с правами на запись).
...
### Deploy из AdminBot
- В узле конструктора «Логи» (раздел «Работа бота») есть две кнопки: `🚀 Deploy` (запускает `/opt/miniden/deploy.sh` в фоне) и `📄 Deploy статус` (показывает PID и хвост логов).
- Логи деплоя пишутся в `/opt/miniden/logs/deploy.log`, PID — в `/opt/miniden/logs/deploy.pid` (папку создавайте вручную с правами на запись).
- Кнопки доступны только администраторам; при повторном нажатии во время работы деплоя бот ответит, что процесс уже идёт.
