...
- При первом взаимодействии с Telegram-ботом создаётся пользователь в БД, привязанный к `telegram_id`.
- Для обычного сайта в браузере остаётся старая авторизация (логин/пароль).
- Если пользователь открывает WebApp из бота (кнопка с WebApp):
  * Telegram передаёт в WebApp подписанные данные `initData`;
  * WebApp отправляет `initData` на `/api/auth/telegram_webapp`;
  * backend проверяет подпись, находит/создаёт пользователя по `telegram_id` и создаёт сессию сайта;
  * за счёт этого WebApp автоматически авторизует пользователя без отдельного логина.
