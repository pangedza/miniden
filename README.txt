...
- 2026-05-XX: Deploy запускается только через `systemctl start miniden-deploy.service` (backend без sudo/tty), статус отдаёт JSON с PID, ActiveState, хвостом `/opt/miniden/logs/deploy.log` и ISO-временем последнего запуска. Действие Deploy убрано из конструктора бота; неизвестные action_type показываются как «Удалено», постоянное Reply-меню использует меню из `/adminbot/menu-buttons` (кнопки «Написать»/«О проекте» работают).
...
- AdminSite (`/adminsite`): конструктор страниц/блоков/шаблонов для витрины и главной страницы.
...
- Ожидаемая команда на сервере: `systemctl start miniden-deploy.service` (юнит дергает `/opt/miniden/deploy.sh`). Deploy запускается только через systemd; в боте нет кнопок/команд для деплоя и backend не использует sudo/tty.
...
---------------------------
- Deploy запускается через системный юнит `/etc/systemd/system/miniden-deploy.service` (User=root, Type=oneshot), который вызывает `/opt/miniden/deploy.sh` и пишет лог в `/opt/miniden/logs/deploy.log`.
- Установка юнита:
  1. `sudo cp deploy/systemd/miniden-deploy.service /etc/systemd/system/`
  2. `sudo systemctl daemon-reload`
  3. `sudo systemctl enable miniden-deploy.service`
- Backend дергает `systemctl start miniden-deploy.service` напрямую (без sudo/tty). Убедитесь, что процесс webapi запущен под пользователем с правами на systemctl (по умолчанию root для юнита miniden-api.service).

Deploy из AdminSite (страница «Работа бота»)
-------------------------------------------
- На странице `/adminbot/runtime` есть секция «Deploy проекта» с двумя кнопками: «🚀 Запустить Deploy» (POST `/admin/deploy/run`) и «📄 Статус (systemd)» (GET `/admin/deploy/status`). Ответы всегда JSON (даже при ошибках доступа).
- Запуск инициируется командой `systemctl start miniden-deploy.service` от пользователя сервиса (root в systemd-юните), без sudo/tty в веб-приложении.
- Статус собирается через `systemctl show/status` + хвост `/opt/miniden/logs/deploy.log` (последние строки и ISO-время модификации файла). UI не ожидает HTML и показывает ошибки из JSON.
