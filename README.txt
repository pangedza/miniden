...
- Стандартизирован ключ `localStorage`: используется `access_token`. Добавлена миграция со старого ключа `miniden_access_token` при первом открытии профиля.
- Добавлен единый токен-хелпер в `webapp/js/api.js`: `getToken()`, `setToken(token)`, `clearToken()` экспортируются в `window` и используются страницей профиля.
- `/profile.html` визуально приведён к общему стилю сайта: контейнер ~1100px, карточки, аккуратные поля, статусы и пустые состояния в iiko-like минимализме.
- Секции `/api/me/addresses`, `/api/me/orders`, `/api/me/cart` теперь:
  - показывают «Загрузка…» во время запроса;
  - при `404/405` выводят «Раздел пока не подключён» без ошибок в консоли;
  - при `401` просят повторно войти.

Как проверить профиль после фикса
---------------------------------
1. Открыть Telegram-бота MiniDeN и выполнить `/login`.
2. Получить одноразовый код.
3. Открыть `/profile.html`.
4. Если токена нет — отображается форма входа (телефон + код).
5. После ввода кода:
   - токен сохраняется в `localStorage.access_token`;
   - отображается карточка профиля и секции «Адреса / Заказы / Корзина».
6. Нажать «Выйти» и убедиться, что снова видна форма входа.
...
Bot: /login — код входа для /profile.html
-----------------------------------------
- Команда `/login` запускает сценарий входа: бот запрашивает телефон, вызывает `POST /api/bot/auth/create-code` и возвращает одноразовый код.
- Телефон нормализуется: удаляются пробелы, дефисы и скобки; если номер начинается на `8` и содержит 11 цифр, он приводится к виду `+7...`.
- При ответе API вида `{ code, expires_in_seconds }` бот отправляет сообщение: `Код для входа: 123456. Действует 5 минут.`
- Если API недоступен или endpoint возвращает `404`, бот отвечает: `Не удалось получить код, попробуйте позже`, а подробности пишет в лог. Для `404` дополнительно выводится подсказка про отсутствие endpoint.

Как тестировать
---------------
1. Открыть Telegram-бота и отправить команду `/login`.
2. Отправить телефон (например, `8 (999) 123-45-67`).
3. Получить сообщение с кодом и временем жизни.
4. Открыть `/profile.html` и ввести телефон + код.
...
Profile.html без Telegram WebApp auth
--------------------------------------
- `webapp/profile.html` теперь работает в обычном браузере без `Telegram.WebApp` и без `initData`.
- Если в `localStorage` нет `access_token`, показывается форма входа: телефон + код.
- После успешного `POST /api/auth/verify-code` токен сохраняется в `localStorage.access_token`, затем вызывается `GET /api/me` и отображается экран профиля.
- Кнопка «Выйти» очищает `access_token` и возвращает форму входа.
- Дополнительно выполняются безопасные запросы:
  - `GET /api/me/addresses`
  - `GET /api/me/orders`
  - `GET /api/me/cart`
  При `404` показываются пустые состояния, при других ошибках страница не падает.

Как проверить в браузере
------------------------
1. Открыть `/profile.html`.
2. Ввести телефон и одноразовый код из бота MiniDeN.
3. Убедиться, что отображаются блоки «Профиль», «Адреса», «Заказы», «Корзина».
4. Нажать «Выйти» и убедиться, что снова видна форма входа.

Изменённые файлы
----------------
- `webapp/profile.html`
- `webapp/css/theme.base.css`
- `README.txt`
...
Bot create-code endpoint
------------------------
- Точное совпадение для бота: `POST /api/bot/auth/create-code` (без завершающего `/`).
- Назначение: бот передаёт телефон и Telegram-пользователя, backend создаёт/обновляет пользователя, привязывает `telegram_id`, инвалидирует предыдущие коды, сохраняет новый код в `login_codes` (в виде hash) и возвращает код в явном виде.
- TTL одноразового кода — 5 минут (`expires_in_seconds = 300`).

Пример запроса:

```bash
curl -X POST http://miniden.ru/api/bot/auth/create-code \
  -H "Content-Type: application/json" \
  -d '{
    "phone": "8 (999) 123-45-67",
    "telegram_id": 123456789,
    "telegram_username": "miniden_user"
  }'
```
...
- Телефон нормализуется к виду `+7...`.
- `telegram_id` привязывается к пользователю по телефону (с merge-логикой при необходимости).
- Если телефон уже привязан к другому `telegram_id`, endpoint вернёт `409 telegram_id_conflict`.
- В ответе возвращается только код (без hash) — это допустимо, потому что endpoint предназначен исключительно для бота и должен возвращать `200`, а не `405`.
...
Удалённая/отключённая логика
----------------------------
- WebApp больше не использует Telegram OAuth/авторизацию по ссылке: оформление заказа работает только через Telegram WebApp + `telegram_id`.
- Форма ввода имени/контакта/комментария в корзине отключена, оформление идёт напрямую в бот.
...
Переход каталога на menu_items
------------------------------
- Основной каталог теперь живёт в таблицах `menu_categories` и `menu_items` (это единственный источник данных для бота и витрины).
- Таблицы `products_baskets` и `products_courses` остаются только для обратной совместимости и архивных сценариев.
- Для миграции старых товаров в новую модель используйте команду:
  `python -c "from services.menu_catalog import migrate_legacy_products_to_menu; migrate_legacy_products_to_menu()"`
  Скрипт создаёт категории (например, «Корзинки», «Курсы»), переносит товары и пропускает уже перенесённые позиции.
...
Сценарии AdminBot (конструктор)
------------------------------
- Узлы сценариев хранятся в `bot_nodes`, кнопки узлов — в `bot_buttons`.
- При сохранении узлов/кнопок в админке автоматически увеличивается `bot_runtime.config_version`, бот подхватывает новые данные без перезапуска (кэш перечитывается при смене версии).
- Событийные шаблоны лежат в `bot_event_triggers` и используются для авто-сообщений (например, `webapp_checkout_created`).
- Автоматизации событий: правила в `bot_automation_rules` запускаются по `trigger_type` и могут отправлять сообщения пользователю/админам, сохранять заказ и подключать пресеты кнопок (`bot_button_presets`).
...
---------------------------------
- Показывается только при доступном `Telegram.WebApp`, иначе скрыта и выводится подсказка "Доступно только в Telegram".
- Отправляет `POST /api/public/checkout/from-webapp` с `tg_user_id`, `items`, `totals` и `client_context`.
...
Корзина: модалка оформления использует `/api/public/checkout/from-webapp`.
Legacy `Telegram.WebApp.sendData` (payload `webapp_order`) считается устаревшим.

WebApp -> Bot: уведомления о заказе
----------------------------------
- Пользователь получает сообщение в Telegram с итогами заказа и кнопками.
- Администратор получает уведомление о каждом заказе.
- Шаблон события `webapp_checkout_created` хранится в `bot_event_triggers` и поддерживает переменные `{order_id}`, `{total}`, `{order_link}` (если ссылка передана в `client_context.order_link`), а также `{items}`, `{qty_total}`, `{currency}`, `{webapp_url}`.
- Автоматизация для оповещения админа создаётся в `initdb.init_db()` при отсутствии правил и использует `bot_automation_rules` с `trigger_type=WEBAPP_ORDER_RECEIVED`.

Корзина: убрана старая телеграм-авторизация, добавлены режимы Browser/Telegram

API endpoint: POST /api/public/checkout/from-webapp
---------------------------------------------------
Запрос (JSON):
  {
    "tg_user_id": 123456,
    "items": [
      {"item_id": 1, "title": "Товар", "qty": 2, "price": 1000, "type": "basket", "category_slug": "korzinki"}
    ],
    "totals": {"qty_total": 2, "sum_total": 2000, "currency": "₽"},
    "client_context": {"page_url": "...", "user_agent": "...", "created_at": "..."}
  }
Ответ:
