...
- Структура сценариев хранится в БД:
  - `bot_nodes` — узлы сценария (MESSAGE/INPUT/CONDITION/ACTION) и связанный текст/настройки.
  - `bot_buttons` — кнопки узлов (callback/url/webapp) с `row`/`position` для раскладки.
  - `bot_actions` — действия внутри узлов (сортировка и параметры).
  - `bot_runtime` — версия конфигурации; после сохранения узла/кнопок бот обновляет кэш при следующем обращении без рестарта.
- События и автоматизация:
  - `bot_event_triggers` — шаблоны сообщений для событий (например, `webapp_checkout_created`), поддерживаются переменные `{order_id}`, `{total}`, `{order_url}`, `{items}`, `{qty_total}` и `{currency}`.
  - `bot_automation_rules` — правила автоматизации (условия + действия). Для WebApp-заказа можно отправлять сообщение пользователю и/или уведомление администратору без перезапуска бота.
...
Корзина: модалка оформления теперь имеет "Оформить в боте" (Telegram.WebApp.sendData).
Формат payload webapp_order.

WebApp -> Bot: обработка web_app_data (type=webapp_order)
--------------------------------------------------------
- Пользователь получает: «✅ Заказ принят» + список товаров + сумма.
- Inline-кнопки пользователю: «Связаться с админом» (tg://user?id=ADMIN_CHAT_ID), «Отменить заказ» (callback `webapp:order:cancel:<order_id>`).
- Админу отправляется карточка: «📦 Новый заказ» + данные пользователя + список товаров + сумма.

Корзина: убрана старая телеграм-авторизация, добавлены режимы Browser/Telegram

API endpoint: POST /api/webapp/order
-----------------------------------
Запрос (JSON):
  {
    "tg_user_id": 123456,
    "cart": {
      "items": [{"id": 1, "title": "Товар", "qty": 2, "price": 1000}],
      "total": 2000,
      "currency": "₽"
    },
