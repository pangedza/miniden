...
-------------------------------------
- В `/cart` кнопка «Оформить заказ» открывает модалку «Оформление заказа»: ссылка «Связаться с админом» берётся из `site_settings.contacts.telegram` или `site_settings.social_links.telegram`; если контакт не задан, кнопка скрывается. Кнопка «Оформить в боте» доступна только внутри Telegram WebApp и отправляет `tg.sendData({ type: 'cart_checkout' })`.
