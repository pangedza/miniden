...
Каталог на сайте
----------------
- `/products.html` показывает товары по категориям из публичного API (`/api/products?type=basket`). Каждая карточка выводит фото, цену, краткое описание и кнопки переходов на маркетплейсы (WB, Ozon, Яндекс, Авито), если ссылки заведены в админке.
- `/masterclasses.html` выводит онлайн мастер-классы (`/api/products?type=course`):
  - бесплатные доступны сразу, кнопка «Перейти к мастер-классу» активна без авторизации;
  - платные видны на витрине, но переход работает только для пользователей с оплаченным доступом; без доступа показывается подсказка и кнопка «Добавить в корзину».
- Обе страницы используют единую структуру данных, которую возвращают `/api/products` и профиль пользователя (masterclass_url, wb_url, ozon_url, yandex_url, avito_url и т.д.).
