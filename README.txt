...
- Доступные роли: `superadmin`, `admin_bot`, `admin_site`.
- Разделы:
  - `/admin/users` — управление администраторами (только superadmin).
  - `/admin/profile` — личный профиль: смена логина и пароля (проверка старого пароля обязательна).
...
- Страница `/adminbot/logs` в админке показывает последние строки файлов логов (переключатель API/BOT, параметр `limit` до 2000).
